...
`Select` renders a dropdown control and sends `{ value = selectedValue }` to Lua when the user chooses an option.
...
| `options` | `string[] | { label, value }[] | reactive getter` | Options. Strings become both label and value. |
| `onChange` | `function(payload)` | Receives `{ value = string }`. |
